package com.alechilles.radialmenu.assets;
...
/**
 * Keeps the embedded Radial Menu asset pack visible to the in-game AssetEditor.
 */
public final class RadialMenuAssetPackCoordinator {

    private static final String ASSET_EDITOR_PLUGIN_CLASS = "com.hypixel.hytale.builtin.asseteditor.AssetEditorPlugin";

    private final RadialMenuMod plugin;

    public RadialMenuAssetPackCoordinator(@Nonnull RadialMenuMod plugin) {
        this.plugin = plugin;
    }
    ...
    public void ensureAssetEditorPackVisible() {
        ...
        if (assetModule == null) {
            plugin.getLogger().at(Level.WARNING).log(
                    "Radial Menu asset editor sync: AssetModule unavailable."
            );
            return;
        }
        ...
        if (pack == null) {
            plugin.getLogger().at(Level.WARNING).log(
                    "Radial Menu asset editor sync: pack '" + packId + "' not found."
            );
            return;
        }
        ...
        try {
            ...
            if (assetEditorPlugin == null) {
                plugin.getLogger().at(Level.INFO).log(
                        "Radial Menu asset editor sync: AssetEditor plugin instance unavailable."
                );
                return;
            }
            ...
            if (getDataSourceForPack.invoke(assetEditorPlugin, packId) != null) {
                plugin.getLogger().at(Level.INFO).log(
                        "Radial Menu asset editor sync: registered read-only data source for pack '" + packId + "'."
                );
                return;
            }

            plugin.getLogger().at(Level.WARNING).log(
                    "Radial Menu asset editor sync: failed to register data source for pack '" + packId + "'."
            );
        } catch (ClassNotFoundException ex) {
            plugin.getLogger().at(Level.INFO).log(
                    "Radial Menu asset editor sync: AssetEditor plugin class not found."
            );
        } catch (Exception ex) {
            plugin.getLogger().at(Level.WARNING).withCause(ex).log(
                    "Radial Menu asset editor sync: failed while registering pack in AssetEditor."
            );
        }
    }
}
